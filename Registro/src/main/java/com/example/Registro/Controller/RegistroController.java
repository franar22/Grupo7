...
public class RegistroController {
    ...
    @Operation(
        ...
        description = "Permite registrar un nuevo usuario con los datos necesarios. Devuelve el usuario registrado si es exitoso."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> registrarUsuario(@Valid @RequestBody UsuarioDTO usuario) {
        try {
            ...
            EntityModel<UsuarioDTO> recurso = EntityModel.of(nuevoRegistro);

            
            recurso.add(linkTo(methodOn(RegistroController.class).registrarUsuario(usuario)).withSelfRel());
            recurso.add(linkTo(methodOn(RegistroController.class).registrarUsuario(usuario)).withRel("registrar"));
            recurso.add(Link.of("http://localhost:8081/api/login").withRel("login"));
